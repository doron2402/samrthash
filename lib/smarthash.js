...
Hash.prototype.checkTtlValue = function(cb){
    ...
    async.forEach(Object.keys(this.Data), function (item, callback){
        if (self.Data[item] && self.Data[item].ttl && self.Data[item].ttl !== null && self.Data[item].ttl <= moment().utc().unix()){
...
Hash.prototype.insert = function(args, cb) {
        ...
        insert_value: function(callback) {
            if (args.ttl) {
                self.Data[args.index] = {
                    value: args.value,
                    ttl: moment().add(time,args.ttl).utc().unix()
                };
            }else {
                self.Data[args.index] = {
                    value: args.value
    ...
    function(err, results) {
        ...
        if (typeof(cb) == 'function'){
            return cb(err, {id: args.index, obj: self.Data[results.insert_value]});
        } else {
    ...
    async.series({
        ...
        fetch: function(callback) {
            if (self.Data[args.index]) {
                ...
                callback(Errors.ValueIsNotFound,null);
            }
        }
    },
    function(err, results) {
...
//args = {index: }
Hash.prototype.fetchAndRemove = function(args, cb) {
    var self = this;
    async.series({
        validate_args: function(callback) {
            if (!args || !args.index) {
        ...
        fetch: function(callback) {
            if (self.Data[args.index]) {
                var value = self.Data[args.index];
                callback(null,value);
            }else {
                callback(Errors.ValueIsNotFound,null);
            }
        },
        remove_index: function(callback) {
            self.remove({index: args.index}, function(err, res){
                callback(err, res);
            });
        },
        update_hash_size: function(callback) {
